...
* $\hat{\bar y}_r$ (red) $\approx \bar y_{U}$ (black), while the naive
...
\qquad
\hat{\bar y}_r = \frac1N\sum_{i=1}^N \hat y_i = \hat B\, \bar x_{U}.
$$

In the figure, this is the height of the red line at $x=\bar x_{U}$:
anchoring it at the known population mean pulls the estimate back near the
true $\bar y_{U}$, despite the sample being biased toward small $x$.
...
$$
$$
\bar y_{U} = \text{average yield in bushels per field}, 
$$
$$
...
* $\hat{\bar y}_{\text{post}}$ (red) $\approx \bar y_{U}$ (black); the
...
\hat{\bar y}_{\text{reg}} = \frac1N\sum_{i=1}^N \hat y_i
 = \hat B_0 + \hat B_1 \bar x_{U}
$$

* $\hat{\bar y}_{\text{reg}}$ (red) $\approx \bar y_{U}$ (black), while
...
$\hat{\bar y}_{\text{ML}}$ (red) $\approx \bar y_{U}$ (black), much
...
x_{U}$ — and SE is usually estimated by the bootstrap.
:::
